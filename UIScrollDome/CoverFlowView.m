...
- (void)adjustReflectionBounds:(CALayer *)layer scale:(CGFloat)scale {
    ...
    reflectLayer.frame = CGRectMake(0, layer.frame.size.height+3, layer.bounds.size.width, layer.bounds.size.height*0.4);
...
- (void)moveOneStep:(BOOL)isSwipingToLeftDirection {
    ...
    if (isSwipingToLeftDirection){
        ...
        int num = (int)self.images.count - self.sideVisibleImageCount - 1;
        if (self.currentRenderingImageIndex < num){
            ...
            if (self.currentRenderingImageIndex == num-1) {
                self.currentRenderingImageIndex = 1;
            }
            //show the layer
        ...
        if (self.currentRenderingImageIndex > self.sideVisibleImageCount){
            ...
            if (self.currentRenderingImageIndex == self.sideVisibleImageCount+1) {
                self.currentRenderingImageIndex = 7;
            }
...
- (void)handleGesture:(UIPanGestureRecognizer *)recognizer {
    if (recognizer.state == UIGestureRecognizerStateChanged){
        ...
        if (fabs(offset.x) > DISTNACE_TO_MAKE_MOVE_FOR_SWIPE) {
            ...
            self.isSwipingToLeftDirection = isSwipingToLeftDirection;
...
+ (id)coverFlowViewWithFrame:(CGRect)frame andImages:(NSMutableArray *)rawImages sideImageCount:(int)sideCount sideImageScale:(CGFloat)sideImageScale middleImageScale:(CGFloat)middleImageScale {
    ...
    flowView.currentRenderingImageIndex = 4;
...
- (void)showImageAndReflection:(CALayer*)layer
{
    ...
    reflectLayer.shadowOpacity = 0.3;
    reflectLayer.frame = CGRectMake(0, layer.frame.size.height+3, layer.bounds.size.width, layer.bounds.size.height*0.4);
...
- (int)imageWithIndex{
    if (self.isSwipingToLeftDirection) {
       
        return self.currentRenderingImageIndex  + self.sideVisibleImageCount;
